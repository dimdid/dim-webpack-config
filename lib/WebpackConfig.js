...
class WebpackConfig {
  ...
  constructor (context = process.cwd(), entries = './index.js', opts = {}) {
    ...
    if (opts.usedOfCss && purgePlug) {
      //remove files from outputPath before
      purgePlug.options.paths = glob.sync(opts.usedOfCss).filter( path => !path.match(wpConf.output.path));
    } else if (!opts.usedOfCss && purgePlug) {
      console.warn('You should use "usedOfCss option, look at ./src/ by default.');
      //not-todo: let me crash..
    }
